...
| `ended` | 재채점 결과 검토, 수동 점수 조정 | 당시 스코어보드 자동 변경 금지 |
...
- 한 참가팀은 하나의 참가 유형만 가진다.
- 참가자는 로그인 후 자기 팀에 지정된 유형 워크스페이스로 이동한다.
- 문제, 제출, 스코어보드, 채점 이력은 유형별로 분리한다.
...
- 대회 중 재채점은 가능하다.
- 대회 운영자/운영매니저는 재채점을 요청할 수 없다.
- 재채점 요청은 `judge.job.retry` 권한을 가진 서비스 관리자 또는 채점 인프라 권한자만 수행한다.
