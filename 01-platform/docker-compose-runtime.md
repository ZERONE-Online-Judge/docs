...
Docker socket mount는 구현 난이도가 낮지만 강한 권한을 준다.
따라서 judge-agent stack은 반드시 별도 judge VM에서만 실행한다.
보안 강화를 위해 추후 systemd agent + rootless Docker 구성을 검토한다.
...
JUDGE_LEASE_EXTEND_INTERVAL_SECONDS=30
```
...
DATABASE_URL=postgresql+asyncpg://zerone:password@postgres:5432/zerone
...
MAIL_QUEUE_POLL_INTERVAL_SECONDS=2
...
```env
APP_ENV=production
INTERNAL_API_BASE_URL=http://backend-vm-tailscale-ip:8000
